...
TwoPlayersBayesianGame::TwoPlayersBayesianGame(){
...
int TwoPlayersBayesianGame::getNombreAgents() {
    return 2;
}

void TwoPlayersBayesianGame::setTypeNumbers(std::vector<std::string> strTypes){
    transform(strTypes.begin(), strTypes.end(), std::back_inserter(typesNumbers),
        [](const std::string& str) { return std::stoi(str); });
}

void TwoPlayersBayesianGame::setGameDimensions(std::vector<std::string> strMatrixDimensions){
    transform(strMatrixDimensions.begin(), strMatrixDimensions.end(), std::back_inserter(gameDimensions),
        [](const std::string& str) { return std::stoi(str); });
}

void TwoPlayersBayesianGame::addPayoffLine(std::vector<std::string> strPayoffs){
...
void TwoPlayersBayesianGame::addJointTypeProbabilities(std::vector<std::string> strProbabilities){
...
float TwoPlayersBayesianGame::getJointTypesProba(std::vector<int> types){
...
float TwoPlayersBayesianGame::getPayoff(std::vector<int> types, std::vector<int> actions, int whichPlayer){
...
std::vector<int> TwoPlayersBayesianGame::getGameDimensions(){
    return gameDimensions;
}

std::vector<int> TwoPlayersBayesianGame::getTypesNumbers(){
    return typesNumbers;
}
